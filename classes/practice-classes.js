...
const ralphsInfo = ralph.getInfo(ralph)

const ralphsDialogue = ralph.dialogue(ralph)
...
class Player extends Character {
  ...
  defend(enemyAttackLevel) {
    const newHealthLevel = this.healthLevel - enemyAttackLevel
    if (newHealthLevel > 0) {
      return {
        attackStrength: enemyAttackLevel,
        remainingHealth: newHealthLevel,
...
fireSpitter.addSuperPower('flaming tongue')
